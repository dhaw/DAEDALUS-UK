function poptim=fitEpi3(ydata,X,data)
%%
[a,b]=size(X);
X=reshape(X',a*b,1);

tvec=[-45.3987,32,87.8544,122,153,183,214,245,275,306,336,367,398,426,457,487,518,548,579];
...
%
ub=[5,0,92,2.5*ones(1,numInt-3)];
x0=[2.8,-57,87,.45,pfit2,pfit3];
lb=[1,-200,61,zeros(1,numInt-3)];
%%
...
function f=sim2fit(params,data,xdata,Xfit)
    ...
    [pr,vx,NN,n,ntot,na,NNbar,NNrep,Dout,beta]=hePrepCovid19(data,numInt,params(1),params(4:end));
